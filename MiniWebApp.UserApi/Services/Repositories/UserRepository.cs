...
public sealed class UserRepository(UserDbContext db, IPasswordHasher<User> _passwordHasher,
    ILogger<UserRepository> _logger, IRequestContext requestContext) : RepositoryBase(requestContext), IUserRepository
{
    public async Task<Outcome<Guid[]>> CreateBulkAsync(IEnumerable<CreateUserRequest> requests, CancellationToken ct = default)
    {
        ...
        {
            return (StatusCodes.Status200OK, Array.Empty<Guid>());
        }
        ...
        return (StatusCodes.Status201Created, newUsers.Select(u => u.Id).ToArray());
    }
    ...
    public async Task<Outcome<Guid>> CreateAsync(CreateUserRequest request, CancellationToken ct = default)
    {
        ...
        if (usernameExists)
        {
            return ("Username already exists.", StatusCodes.Status409Conflict);
        }
        ...
        return (StatusCodes.Status201Created, user.Id);
    }
    ...
    public async Task<Outcome<AuthenticatedUserContext>> VerifyCredentialsAsync(VerifyCredentialsRequest request, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(identifier))
        {
            ...
            return ("Username or email is required.", StatusCodes.Status400BadRequest);
        }
        ...
        if (user.LockoutEnd > DateTime.UtcNow)
        {
            ...
            return ("Account is temporarily locked.", StatusCodes.Status423Locked);
        }
        ...
        return (StatusCodes.Status200OK, userTokenContext);
    }
